...
import {
  ...
  export default function GameScene(engine: Engine) {
    ...
    //initialise physics
    that.scene.enablePhysics(new Vector3(0, 0.01, 0), havokPlugin);
    //----------------------------------------------------------

    //any further code goes here-----------
    that.box = createBox(that.scene, 2, 1, 2);
    that.box = createBox(that.scene, 1, 1, 1);
    that.box = createBox(that.scene, -1, 1, -1);
    that.box = createBox(that.scene, -2, 1, -2);
    that.box = createBox(that.scene, 1, 1, -3);
    that.box = createBox(that.scene, -3, 1, 1);
    that.box = createBox(that.scene, -1, 1, 2);
